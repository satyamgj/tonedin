...
router.post("/", async (req, res) => {
  try {
    const { skinAnalysis, imageBase64, mimeType, specifications } =
      req.body ?? {};

    if (!skinAnalysis) {
      return res.status(400).json({ error: "Complete skin analysis first." });
    }
...
Score how well this product suits this user. You must return matchPercentage as an integer 0-100. Use the product image to identify the true pigment color.`;
    ...
    res.json({
      match,
      product: { brand, productName, category },
    });
  } catch (err) {
    ...
    const message =
      err instanceof ModelResponseError
        ? err.message
        : err.message || "Product matching failed";
